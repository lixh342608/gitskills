...
class appfilter(logging.Filter):
    ...
    def filter(self,record):
        ...
        return True
class globallogger:
    def __init__(self,name="root",log_file=None,level=logging.INFO):
        self.log=logging.getLogger(name+''.join([random.choice(string.digits+string.ascii_letters)for _ in range(8)]))
        self.log.addFilter(appfilter(name=name))
        ch=logging.StreamHandler(sys.stdout)
        fmt='%(levelname)s %(asctime)s-%(name)s-%(filename)s:%(funcName)s:%(lineno)s-%(message)s'
        datefmt='%Y-%m-%d %H:%M:%S'
        ...
        if (log_file):
            fh=multiprocesshandler(log_file,when="D",backupCount=7)
